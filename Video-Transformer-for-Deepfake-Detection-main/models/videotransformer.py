...
#       Positional embedding
    ...
    def forward(self, x):
        ...
        """
        x = x.transpose(1, 2)
        batch, num_images, channels, height, width = x.shape
        if self.hybrid == False:
            ...
            for batch_number in range(batch):
                ...
                for images in range((num_images)):
                    x_out = DDFA.generate_uv_tex(x[batch_number][images].unsqueeze(0), self.patch_embedding, self.sequence_embedding, self.linear_1, self.linear_3, self.proj, self.seq_embed, self.hybrid, self.variant, self.device)
            ...
            for batch_number in range(batch):
                ...
                for images in range((num_images)):
                    print(images)
                    x_out = DDFA.generate_uv_tex(x[batch_number][images].unsqueeze(0), self.xception_feature_extractor, self.sequence_embedding, self.linear_1, self.linear_3, self.proj, self.seq_embed, self.hybrid, self.variant, self.device)
                    # x = x.transpose(2,1)
                    # x = self.linear_3(x)
                    # x = x.transpose(2,1)
                    x_out = self.sequence_embedding(x_out)
