...
from LipForensics_mix.data_process.transform import ToTensorMouth, NormalizeMouth
...
# 自定义人脸特征点检测和嘴部区域截取函数
# 自定义人脸特征点检测和嘴部区域截取函数
def detect_mouth_region(video_tensor):
    ...
    for frame_idx, frame in enumerate(video_tensor.permute(1, 0, 2, 3)):  # 交换维度，变为 (t, c, h, w)
        ...
        frame = frame.cpu().permute(1, 2, 0).numpy()  # 交换维度，变为 (h, w, c)
        if frame.dtype != np.uint8:
            frame = (frame * 255).astype(np.uint8)  # 将浮点数转换为 0-255 的 uint8
        ...
        if len(faces) > 0:
            ...
            # 将灰度图转换为 Tensor
            mouth_region_gray_resized = torch.from_numpy(mouth_region_gray_resized).float()
            mouth_region_gray_resized = mouth_region_gray_resized.unsqueeze(0)  # 添加通道维度
        else:
            # 如果未检测到人脸，返回空张量
            mouth_region_gray_resized = torch.zeros(1, 48, 48)  # 假设嘴部区域大小为 48x48 的零张量
...
def tensor_process(batch,device):
    # 检查 batch 中的值数量
        if len(batch) == 2:
            data, labels = batch
            lengths = torch.tensor([data.size(1)] * data.size(0), dtype=torch.long)  # 创建一个默认的 lengths 张量
        elif len(batch) == 3:
        ...
        # 对每个视频张量进行处理
        processed_data = [detect_mouth_region(video) for video in data]
        processed_data = pad_sequence(processed_data, batch_first=True, padding_value=0)

        # 更新 lengths，确保其与处理后的数据一致
        lengths = torch.tensor([min(len(video), 9) for video in processed_data], dtype=torch.long)  # 假设最多处理 9 帧

        processed_data, labels, lengths = processed_data.to(device), labels.to(device), lengths.to(device)
